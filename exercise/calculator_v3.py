...
def num1Click(event):
    interactWithScreen(str(1))

def num2Click(event):
    interactWithScreen(str(2))

def num3Click(event):
    interactWithScreen(str(3))

def num4Click(event):
    interactWithScreen(str(4))

def num5Click(event):
    interactWithScreen(str(5))

def num6Click(event):
    interactWithScreen(str(6))

def num7Click(event):
    interactWithScreen(str(7))

def num8Click(event):
    interactWithScreen(str(8))

def num9Click(event):
    interactWithScreen(str(9))

def num0Click(event):
    interactWithScreen(str(0))
...
def numMultiClick(event):
    interactWithScreen('x')

def numDivClick(event):
    interactWithScreen(chr(247))
...
def generate_buttons(container1_buttons):
    symbols_list = ['C',chr(247),'x','-','+','.','=']

    buttonClear = Button(container1_buttons)
    # Setting the attributes
    buttonClear["text"] = symbols_list[0]
    buttonClear["background"] = COLOT_BTN
    buttonClear.grid(column=1)
    buttonClear.config(height = HEIGHT_BTN, width = WIDTH_BTN)
    buttonClear.bind(numClearClick)

    for row_num in range(5):
        for col_num in range(4):
            if(row_num+1 != 5):
                print(str(row_num+1) + " and " + str(col_num))
